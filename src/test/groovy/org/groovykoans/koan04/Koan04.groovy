...
class Koan04 extends GroovyTestCase {
    ...
    void test01_IntroToClosures() {
        ...
        expectedHelloRonalda = helloRonalda
        ...
        def resultClosure = {int a,b -> 2*(a+b)}
        // ------------ START EDITING HERE ----------------------
    ...
    void test02_MoreClosureIntro() {
        ...
        def file1 = new File("$prefix/exercise.txt")

        //filteredResult = file1.readLines()
        //filteredResult.findAll({x -> x==~ /^#/})

       // println filteredResult.toString()

        file1.filterLine(filteredResult) {x -> !(x=~ /^#/) }

       // filteredResult = file1.readLines().toString().eachMatch(/#\\S+/){}
        //filteredResult = file1.readLines().toString().replaceAll(/#\\S+/, '\n')
